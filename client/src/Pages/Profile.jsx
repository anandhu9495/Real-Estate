...
import { updateUserStart , updateUserSuccess, updateUserFailure, deleteUserFailure, deleteUserStart, deleteUserSucess, signOutUserStart, signOutUserFailure, signOutUserSucess} from '../redux/user/userSlice'
    ...
    <div className='p-3 max-w-lg mx-auto'>
      ...
      <form onSubmit={handleSubmit} className='flex flex-col gap-4'>
        ...
        <p className='text-sm self-center'>
          {fileUploadError ? (
          <span className='text-red-700'>Error image Upload (image must be less than 2 mb)</span> ) :  fileperc > 0 && fileperc < 100 ? ( <span className='text-green-700'>{`uploading ${fileperc} %`}</span> ) :  fileperc === 100 ? ( <span className='text-green-700'>Uploaded successfully !</span> ) : ''
          }
        </p>
      ...
      {userListings && userListings.length>0 && userListings.map((listing) => (
        ...
        <div key={listing._id} className='border rounded-lg p-3 flex justify-between items-center gap-4'>
          ...
          <Link to={`/listing/${listing._id}`}>
            ...
            <img src={listing.imageUrls} alt="cover" className='h-16 w-16 object-contain ' />
